...
class CPDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # Định nghĩa hai transform khác nhau: một cho hình ảnh RGB và một cho grayscale
    ...
    def transform_image(self, img):
        """Hàm giúp áp dụng transform phù hợp dựa vào số kênh màu"""
        if img.mode == 'L' or img.mode == '1':  # Grayscale
    ...
    def __getitem__(self, index):
        ...
        if self.stage == 'GMM':
            ...
            c = Image.open(osp.join(self.data_path, 'warp_cloth', im_name))    # c_name, if that is used when saved
            cm = Image.open(osp.join(self.data_path, 'warp_mask', im_name)).convert('L')    # c_name, if that is used when saved
        ...
        im_parse = Image.open(
            osp.join(self.data_path, 'image_parse', parse_name)).convert('L')   # updated new segmentation
        ...
        # Tạo mặt nạ tóc riêng biệt để xử lý vấn đề áo dính lên tóc
        parse_hair = (parse_array == 2).astype(np.float32)  # Chỉ phần tóc
        hair_mask = torch.from_numpy(parse_hair)  # [0,1]

        # Thêm mặt nạ cổ/da (neck/skin mask) - class 20 theo bản phân đoạn mới
        parse_neck = (parse_array == 20).astype(np.float32)  # Chỉ phần cổ và da
        # neck_mask = torch.from_numpy(parse_neck)  # [0,1]

        # Kết hợp mặt nạ tóc và cổ để tránh áo dính lên các vùng này
        face_hair_neck_mask = (parse_array == 2).astype(np.float32) + \
                              (parse_array == 13).astype(np.float32) + \
                              (parse_array == 20).astype(np.float32)  # Tóc + Mặt + Cổ
        protected_mask = torch.from_numpy(face_hair_neck_mask)  # [0,1]
        ...
        # Sử dụng transform_gray cho cả hai hình ảnh shape vì đều là grayscale
        ...
        if self.stage == 'GMM':
            ...
            im_g = self.transform_rgb(im_g)
        else:
            im_g = ''

        pcm.unsqueeze_(0)  # CP-VTON+
        hair_mask.unsqueeze_(0)  # Thêm mask tóc vào tensor
        # neck_mask.unsqueeze_(0)  # Thêm mask cổ vào tensor
        protected_mask.unsqueeze_(0)  # Thêm mask bảo vệ tóc+mặt+cổ vào tensor
        ...
        result = {
            ...
            'hair_mask': hair_mask,     # Mask tóc để tránh áo dính lên tóc
            # 'neck_mask': neck_mask,     # Mask cổ để tránh áo dính lên cổ
            'protected_mask': protected_mask,  # Mask kết hợp tóc+mặt+cổ để bảo vệ khỏi áo
        }
