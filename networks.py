...
class FeatureExtraction(nn.Module):
    def __init__(self, input_nc, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_dropout=False, device='cuda'):
...
class FeatureRegression(nn.Module):
    def __init__(self, input_nc=512, output_dim=6, device=None):
        ...
        self.tanh = self.tanh.to(self.device)
...
class TpsGridGen(nn.Module):
    def __init__(self, out_h=256, out_w=192, use_regular_grid=True, grid_size=3, reg_factor=0, device=None):
...
class VGGLoss(nn.Module):
    def __init__(self, layids=None, device=None):
...
class GMM(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.device = opt.device if hasattr(opt, 'device') else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.extractionA = FeatureExtraction(
            22, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d, device=self.device)
        self.extractionB = FeatureExtraction(
            1, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d, device=self.device)
        self.l2norm = FeatureL2Norm()
        self.correlation = FeatureCorrelation()
        self.regression = FeatureRegression(
            input_nc=192, output_dim=2*opt.grid_size**2, device=self.device)
        self.gridGen = TpsGridGen(
            opt.fine_height, opt.fine_width, device=self.device, grid_size=opt.grid_size)
...
def save_checkpoint(model, save_path):
    ...
    # Lưu lại thiết bị hiện tại của model
    device = next(model.parameters()).device
    
    # Chuyển model về CPU để lưu
    torch.save(model.cpu().state_dict(), save_path)
    
    # Chuyển model trở lại thiết bị ban đầu
    model.to(device)
...
def load_checkpoint(model, checkpoint_path):
    if not os.path.exists(checkpoint_path):
        return
    device = next(model.parameters()).device  # Lấy device hiện tại của mô hình
